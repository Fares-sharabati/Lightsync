...
export default function SportsScreen() {
  ...
  const awayColor = game?.awayTeam.primaryColor || '#FFFFFF';
  ...
  return <main className="audience-screen" style={{ background: '#050607' }}>
    <section className="audience-content" style={{ width: 'min(94vw, 1400px)', height: '92dvh' }}>
      ...
      {game && <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 'clamp(14px, 3vw, 36px)', margin: '10px 0 24px', fontWeight: 900 }}><div style={{ textAlign: 'center' }}>{game.homeTeam.logoUrl && <img src={game.homeTeam.logoUrl} alt="" style={{ width: 'clamp(42px, 7vmin, 88px)', height: 'clamp(42px, 7vmin, 88px)', objectFit: 'contain', display: 'block', margin: '0 auto 6px' }} />}<div style={{ borderBottom: `4px solid ${homeColor}`, paddingBottom: 5 }}>{game.homeTeam.name}</div></div><span style={{ opacity: .5 }}>VS</span><div style={{ textAlign: 'center' }}>{game.awayTeam.logoUrl && <img src={game.awayTeam.logoUrl} alt="" style={{ width: 'clamp(42px, 7vmin, 88px)', height: 'clamp(42px, 7vmin, 88px)', objectFit: 'contain', display: 'block', margin: '0 auto 6px' }} />}<div style={{ borderBottom: `4px solid ${awayColor}`, paddingBottom: 5 }}>{game.awayTeam.name}</div></div></div>}
