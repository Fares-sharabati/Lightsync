...
export default function Join() {
  ...
  return <main className="light-page" style={{ background: activeBackground, color: activeText, transition: 'background-color .08s linear, color .08s linear' }}><div className="light-content"><div className="light-logo">LIGHTSYNC</div><div className="light-event-name">{event.name}</div>{sportsGame && <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginTop: 10, fontSize: 14, fontWeight: 800, opacity: .9 }}><span>{sportsGame.homeTeam.name}</span><span>vs</span><span>{sportsGame.awayTeam.name}</span></div>}<div className="light-status">CONNECTED</div>{running ? <><div className="show-live-text">SHOW LIVE</div><div style={{ fontSize: '4rem', marginTop: 30 }}>{lightState ? 'ON' : 'OFF'}</div><p className="waiting-description">Your flashlight and screen are synchronized with the show.</p></> : <><div className="connected-icon">✓</div><div className="waiting-message">{event.status === 'finished' ? 'SHOW FINISHED' : 'READY'}</div><p className="waiting-description">{event.status === 'finished' ? 'This LightSync show has finished.' : 'Waiting for the organizer.'}</p></>}</div></main>;
}
