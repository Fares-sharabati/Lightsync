...
export default function Join() {
  ...
  useEffect(() => {
    if (!eventId) {
      setLoaded(true);
      setError('Invalid show link. Please scan the QR code again.');
      return;
    }
    ...
    async function connectToShow() {
      try {
        // Authentication must complete before reading publicShows because the
        // Firebase rules require auth != null for that path.
        ...
        if (!cancelled) {
          setLoaded(true);
          setError('Could not connect to LightSync. Please refresh and scan the QR code again.');
        }
      }
    }

    // Never leave a visitor on an infinite Loading show... screen.
    loadingTimeout = window.setTimeout(() => {
      if (!cancelled && !loaded) {
        setLoaded(true);
        setError('The show is taking too long to load. Please scan the QR code again.');
      }
    }, 10000);
  ...
  async function setFlash(enabled: boolean) {
    ...
    try {
      await track.applyConstraints({
        advanced: [{ torch: enabled } as TorchConstraints],
      });
  ...
  async function joinShow() {
    ...
    try {
      ...
      console.error(err);
      setError(
        err instanceof Error && err.message.includes('Torch')
          ? 'This phone/browser does not allow flashlight control. Try the latest Safari or Chrome.'
          : 'Please allow camera access so LightSync can control your flashlight.'
      );
    }
  }
    ...
    return () => {
      ...
      if (trackRef.current) {
        void trackRef.current
          .applyConstraints({ advanced: [{ torch: false } as TorchConstraints] })
          .catch(() => {});
  ...
  const screenColor = /^#[0-9a-fA-F]{6}$/.test(event.screenLightColor || '')
    ? event.screenLightColor!
    : DEFAULT_SCREEN_LIGHT_COLOR;
  ...
  return (
    ...
    >
      <div className="light-content">
        ...
        {running ? (
          <>
            <div className="show-live-text">SHOW LIVE</div>
            <div style={{ fontSize: '4rem', marginTop: 30 }}>{lightState ? 'ON' : 'OFF'}</div>
            <p className="waiting-description">Your flashlight and screen are synchronized with the show.</p>
          </>
        ) : (
          <>
            <div className="connected-icon">✓</div>
            <div className="waiting-message">READY</div>
            <p className="waiting-description">Waiting for the organizer.</p>
          </>
        )}
      </div>
    </main>
