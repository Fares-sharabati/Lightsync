...
export default function Admin() {
  ...
  const [shows, setShows] = useState<Show[]>([]); const [name, setName] = useState(''); const [date, setDate] = useState(''); const [venue, setVenue] = useState('');
  ...
  async function handleCreateShow() { if (!auth.currentUser) return; if (!name.trim() || !date || !venue.trim()) { setMessage('Please enter the show name, date and venue.'); return; } setCreating(true); setMessage(''); try { const id = await createShow(auth.currentUser.uid, { name, date, venue }); setName(''); setDate(''); setVenue(''); navigate(`/admin/show/${id}`); } catch (error) { console.error(error); setMessage('Could not create the show.'); } finally { setCreating(false); } }
  async function handleDeleteShow(show: Show) { if (!auth.currentUser || deletingId) return; if (!window.confirm(`Delete “${show.name}”?\n\nThis permanently removes the show, its public QR data, audience records and statistics. This cannot be undone.`)) return; setDeletingId(show.id); setMessage(''); try { await deleteShow(show.id); setMessage(`“${show.name}” was deleted.`); } catch (error) { console.error(error); setMessage('Could not delete the show. Check your Firebase rules and try again.'); } finally { setDeletingId(null); } }
  ...
  return <main className="ls-shell" style={organizerTheme}>
    ...
    <section className="ls-card"><div className="ls-section-title"><div><p className="ls-eyebrow">SHOW HISTORY</p><h2>Your shows</h2></div><span className="ls-count">{shows.length}</span></div>
      {shows.length === 0 ? <div className="ls-empty">No shows yet. Create your first show above.</div> : <div className="ls-show-list">{shows.map(show => <div className="ls-show-row" key={show.id} style={{ cursor: 'default', padding: 0, gap: 8 }}>
        <button className="ls-show-main" style={{ flex: 1, border: 0, background: 'transparent', color: 'inherit', cursor: 'pointer', textAlign: 'left', padding: '17px' }} onClick={() => navigate(`/admin/show/${show.id}`)} aria-label={`Open ${show.name}`}><div><strong>{show.name}</strong><span>{show.venue} · {show.date}</span></div><div><span className={`ls-status ls-${show.status}`}>{show.status}</span><b>→</b></div></button>
        <button className="ls-delete-show" style={{ marginRight: 10, border: '1px solid #45494e', background: '#111315', color: '#c9cdd2', borderRadius: 8, padding: '8px 10px', fontSize: 10, fontWeight: 800, letterSpacing: '.08em', cursor: 'pointer' }} onClick={() => void handleDeleteShow(show)} disabled={deletingId === show.id}>{deletingId === show.id ? 'DELETING…' : 'DELETE'}</button>
      </div>)}</div>}
    </section>
  </main>;
}
