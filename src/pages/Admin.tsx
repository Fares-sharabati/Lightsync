...
export default function Admin() {
  ...
  const teamFields = (side: 'home' | 'away', team: SportsTeam, setTeam: React.Dispatch<React.SetStateAction<SportsTeam>>) => <div style={{ border: '1px solid var(--line)', borderRadius: 14, padding: 16 }}><p className="ls-eyebrow">{side === 'home' ? 'HOME TEAM' : 'AWAY TEAM'}</p><input className="ls-input" style={{ marginTop: 8 }} placeholder={side === 'home' ? 'Gaziantep BB' : 'Kipas SK'} value={team.name} onChange={e => setTeam(value => ({ ...value, name: e.target.value }))} /><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 10 }}><label className="ls-muted">PRIMARY<input type="color" value={team.primaryColor} onChange={e => setTeam(value => ({ ...value, primaryColor: e.target.value.toUpperCase() }))} style={{ display: 'block', width: '100%', height: 40, marginTop: 4 }} /></label><label className="ls-muted">SECONDARY<input type="color" value={team.secondaryColor} onChange={e => setTeam(value => ({ ...value, secondaryColor: e.target.value.toUpperCase() }))} style={{ display: 'block', width: '100%', height: 40, marginTop: 4 }} /></label></div></div>;
