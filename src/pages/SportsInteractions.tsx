...
export default function SportsInteractions() {
  ...
  function openBigScreen() {
    if (!eventId) return;
    // Reuses the same tab on repeat clicks instead of piling up new ones —
    // window.open with a stable name focuses the existing tab if it's
    // already open, or opens a fresh one if it isn't.
    window.open(`/sports-screen/${eventId}`, 'lightsync-sports-screen', 'noopener,noreferrer');
  }
