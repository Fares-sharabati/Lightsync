...
export default function SportsInteractions() {
  ...
  return <main className="ls-shell" style={{ minHeight: '100vh', padding: 24 }}>
    <header className="ls-header"><div><div className="ls-brand">LIGHTSYNC</div><p className="ls-eyebrow">SPORTS INTERACTIONS</p></div><div style={{ display: 'flex', gap: 10 }}><button className="ls-button ls-primary" onClick={openBigScreen}>OPEN BIG SCREEN ↗</button><button className="ls-button ls-secondary" onClick={() => navigate(`/admin/event/${eventId}`)}>BACK TO EVENT</button></div></header>
    <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gap: 18 }}>
      {game && <section className="ls-card" style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}><div style={{ flex: 1 }}><p className="ls-eyebrow">SPORTS EVENT · {game.sport.toUpperCase()}</p><h2 style={{ margin: '5px 0' }}>{game.homeTeam.name} <span className="ls-muted">vs</span> {game.awayTeam.name}</h2><p className="ls-muted">Team branding is configured when the sports event is created.</p></div>{game.homeTeam.logoUrl && <img src={game.homeTeam.logoUrl} alt="Home team logo" style={{ width: 58, height: 58, objectFit: 'contain' }} />}{game.awayTeam.logoUrl && <img src={game.awayTeam.logoUrl} alt="Away team logo" style={{ width: 58, height: 58, objectFit: 'contain' }} />}</section>}
