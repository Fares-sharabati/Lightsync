...
export default function EventControl() {
  ...
  const [customColor, setCustomColor] = useState('#071B3A');
  ...
  async function changeScreenColor(color: string) { if (!/^#[0-9a-fA-F]{6}$/.test(color) || !eventId || color.toLowerCase() === event?.screenLightColor?.toLowerCase()) return; try { await updateShow(eventId, { screenLightColor: color.toUpperCase() }); setCustomColor(color.toUpperCase()); } catch (error) { console.error(error); setAnalysisMessage('Could not change the audience screen color.'); } }
  function applyCustomColor() { const value = customColor.trim().startsWith('#') ? customColor.trim() : `#${customColor.trim()}`; if (!/^#[0-9a-fA-F]{6}$/.test(value)) { setAnalysisMessage('Enter a valid 6-digit HEX color, for example #071B3A.'); return; } setAnalysisMessage(''); void changeScreenColor(value); }
  ...
  return <main className="ls-shell ls-event-shell">
    ...
    <section className="ls-event-grid">
      ...
      <div className="ls-card" style={{ gridColumn: '1 / -1' }}>
        ...
        <p className="ls-muted" style={{ marginTop: 8 }}>Choose any color for audience phone screens. Use the full browser color palette or enter an exact HEX code. The physical phone flashlight remains white.</p>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginTop: 18 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderRadius: 14, border: '1px solid var(--line)', background: 'var(--panel)', cursor: 'pointer' }}>
            <input type="color" value={screenColor} onChange={e => void changeScreenColor(e.target.value)} aria-label="Choose any screen light color" style={{ width: 46, height: 38, padding: 0, border: 0, background: 'transparent', cursor: 'pointer' }} />
            <span style={{ fontWeight: 800 }}>COLOR PALETTE</span>
          </label>
