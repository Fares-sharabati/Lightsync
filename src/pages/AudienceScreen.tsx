...
export default function AudienceScreen() {
  ...
  if (!loaded) return <main className="audience-screen"><div className="audience-loading">LIGHTSYNC<br/><span>{t({ tr: 'YÃ¼kleniyor...', en: 'Loading...' })}</span></div></main>;
  if (!show || !eventId) return <main className="audience-screen"><div className="audience-loading">{t({ tr: 'ETKÄ°NLÄ°K BULUNAMADI', en: 'SHOW NOT FOUND' })}</div></main>;
  ...
  return (
    <main className="audience-screen">
      ...
      <section className="audience-content">
        ...
        <p className="audience-eyebrow">{t({ tr: 'IÅIK GÃ–STERÄ°SÄ°', en: 'AUDIENCE LIGHT SHOW' })}</p>
        <h1>{show.name}</h1>
        ...
        <h2>{t({ tr: 'KATILMAK Ä°Ã‡Ä°N TARAT', en: 'SCAN TO JOIN' })}</h2>
        <p className="audience-instruction">{t({ tr: 'Telefonunuzun kamerasÄ±nÄ± QR koduna tutun ve Ä±ÅŸÄ±k gÃ¶sterisine katÄ±lÄ±n.', en: 'Point your phone camera at the QR code and join the crowd light show.' })}</p>
