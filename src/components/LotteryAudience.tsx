...
export default function LotteryAudience() {
  ...
  useEffect(() => {
    if (!eventId) return;
    let active = true;
    void ensureAnonymousAuth().then(user => { if (active) setUid(user.uid); }).catch(console.error);
    return () => { active = false; };
  }, [eventId]);
