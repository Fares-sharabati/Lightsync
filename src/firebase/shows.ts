...
function normalizeShow(id: string, value: Record<string, unknown>): Show { return { id, ...(value as Omit<Show, 'id' | 'kind'>), kind: value.kind === 'sports' ? 'sports' : 'show' } as Show; }
function normalizePublicShow(id: string, value: Record<string, unknown>): PublicShow { return { id, ...(value as Omit<PublicShow, 'id' | 'kind'>), kind: value.kind === 'sports' ? 'sports' : 'show' } as PublicShow; }

export async function createShow(organizerId: string, input: CreateShowInput) {
  const showRef = push(ref(db, 'shows')); const showId = showRef.key; if (!showId) throw new Error('Could not create show ID.');
  ...
  await set(showRef, show); await set(ref(db, `publicShows/${showId}`), { name: show.name, date: show.date, venue: show.venue, kind: show.kind, status: show.status, showStartTime: null, lightTimeline: null, screenLightColor: show.screenLightColor }); await set(ref(db, `showStats/${showId}`), { totalScans: 0, totalJoined: 0, peakConnected: 0 }); return showId;
}
...
export async function deleteShow(showId: string) { await update(ref(db), { [`shows/${showId}`]: null, [`publicShows/${showId}`]: null, [`showParticipants/${showId}`]: null, [`showStats/${showId}`]: null, [`sportsGames/${showId}`]: null, [`sportsInteractions/${showId}`]: null, [`sportsResponses/${showId}`]: null, [`sportsResults/${showId}`]: null, [`sportsScreen/${showId}`]: null }); }
