...
export type Show = { id: string; organizerId: string; name: string; date: string; venue: string; status: ShowStatus; kind: 'sports'; createdAt: number; showStartTime?: number | null; lightTimeline?: unknown; screenLightColor?: string; phoneUiColor?: string };
...
export async function createShow(organizerId: string, input: CreateShowInput) {
  ...
  const now = Date.now();
  const show = { organizerId, name: input.name.trim(), date: input.date, venue: input.venue.trim(), kind: 'sports' as const, status: 'waiting' as ShowStatus, createdAt: now, showStartTime: null, screenLightColor: '#FFFFFF', phoneUiColor: '#FFFFFF' };
...
export async function updateShow(showId: string, changes: Partial<Omit<Show, 'id' | 'organizerId'>>) { const updates: Record<string, unknown> = {}; for (const [key, value] of Object.entries(changes)) updates[`shows/${showId}/${key}`] = value; for (const key of ['name', 'date', 'venue', 'kind', 'status', 'showStartTime', 'lightTimeline', 'screenLightColor', 'phoneUiColor']) if (key in changes) updates[`publicShows/${showId}/${key}`] = (changes as Record<string, unknown>)[key]; await update(ref(db), updates); }
