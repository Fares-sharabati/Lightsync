...
export type Show = { id: string; organizerId: string; name: string; date: string; venue: string; status: ShowStatus; createdAt: number; showStartTime?: number | null; lightTimeline?: unknown; screenLightColor?: string };
...
export async function createShow(organizerId: string, input: CreateShowInput) {
  ...
  const show = { organizerId, name: input.name.trim(), date: input.date, venue: input.venue.trim(), status: 'waiting' as ShowStatus, createdAt: Date.now(), showStartTime: null, screenLightColor: '#071B3A' };
  await set(showRef, show);
  await set(ref(db, `publicShows/${showId}`), { name: show.name, date: show.date, venue: show.venue, status: show.status, showStartTime: null, lightTimeline: null, screenLightColor: show.screenLightColor });
...
export async function updateShow(showId: string, changes: Partial<Omit<Show, 'id' | 'organizerId'>>) {
  ...
  for (const key of ['name', 'date', 'venue', 'status', 'showStartTime', 'lightTimeline', 'screenLightColor']) if (key in changes) updates[`publicShows/${showId}/${key}`] = (changes as Record<string, unknown>)[key];
