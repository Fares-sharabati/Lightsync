...
export async function createShow(organizerId: string, input: CreateShowInput) {
  ...
  try {
    await set(ref(db, `publicShows/${showId}`), { name: show.name, date: show.date, venue: show.venue, kind: 'sports', status: show.status, showStartTime: null, lightTimeline: null, screenLightColor: show.screenLightColor });
    await set(ref(db, `showStats/${showId}`), { totalScans: 0, totalJoined: 0, peakConnected: 0 });
  } catch (error) {
    // Roll back a partially-created event so the dashboard never shows a broken event.
...
export async function deleteShow(showId: string) {
  // Delete children first while the owner record still exists, then delete the owner last.
