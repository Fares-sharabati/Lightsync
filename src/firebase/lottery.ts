...
export function watchLottery(showId: string, callback: (lottery: LotteryState | null) => void): Unsubscribe {
  return onValue(ref(db, `lotteries/${showId}`), snapshot => {
    callback(snapshot.exists() ? snapshot.val() as LotteryState : null);
  });
}

export function watchLotteryContact(showId: string, uid: string, callback: (contact: LotteryContact | null) => void): Unsubscribe {
  return onValue(ref(db, `lotteryContacts/${showId}/${uid}`), snapshot => {
    callback(snapshot.exists() ? snapshot.val() as LotteryContact : null);
  });
}
...
export async function startLottery(showId: string, winnerIds: string[], eligibleCount: number, winnerCount: number) {
  ...
  await set(ref(db, `lotteries/${showId}`), {
    ...
    winnerIds: Object.fromEntries(winnerIds.map(uid => [uid, true])),
...
export async function submitLotteryContact(showId: string, uid: string, contact: Omit<LotteryContact, 'submittedAt'>) {
  await set(ref(db, `lotteryContacts/${showId}/${uid}`), {
    ...contact,
